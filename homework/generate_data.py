...
def generate_fake_drivers(n):
    ...
    """
    drivers = []
    for i in tqdm(range(n), desc="Generating drivers"):
        record = {
            "driverId": i + 10,
            "name": fake.name(),
            "ssn": fake.ssn(),
...
if __name__ == "__main__":
    fakeDrivers = generate_fake_drivers(100)
    fake_timesheet = generate_fake_timesheet(fakeDrivers, 1000)

    if not os.path.exists("files"):
        os.makedirs("files")
    save_fake_data(fakeDrivers, "files/drivers.csv")
